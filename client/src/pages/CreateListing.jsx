     ...
     if(files.length > 0 && files.length + formData.imageUrls.length < 7) {
        ...
        setImageUploadError(false);
         ...
         for (let i = 0; i < files.length; i++) {
            // Check if the file is an image and is less than 2MB
            if (files[i].type.startsWith('image/') && files[i].size < 2 * 1024 * 1024) {
              promises.push(storeImage(files[i]));
            } else {
              setImageUploadError('Only image files under 2MB are allowed');
              setUploading(false);
              return;
            }
          }
      ...
      <form className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
           ...
           <div className="flex gap-4">
                <input onChange={(e)=>setFiles(e.target.files)} className="p-3 border-gray-300 rounded" type="file" id="images" accept="image/*" multiple/>
